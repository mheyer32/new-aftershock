...
static char *SkipWhitespace( char *data, unsigned char *hasNewLines )
{
	int c;
...
char *COM_ParseExt( char **data_p, aboolean allowLineBreaks )
{
	int c = 0, len;
	unsigned char hasNewLines = 0;
	char *data;
...
If "allowLineBreaks" is 1 then an empty
...
void SkipBracedSection (char **program) {
	...
	depth = 0;
	do {
		token = COM_ParseExt( program, atrue );
		if( token[1] == 0 ) {
			if( token[0] == '{' ) {
				depth++;
			}
...
void COM_ExtractFilePath(char *path, char *dest)
{
	char *src;
...
void COM_ExtractFileBase(char *path, char *dest)
{
	char *src;
...
void COM_ExtractFileExtension(char *path, char *dest)
{
	char *src;
...
void A_strcat( char *dest, int size, const char *src )
{
	...
	l1 = strlen( dest );
	if ( l1 >= size ) {
		Com_Error( ERR_FATAL, "G_strcat: already overflowed" );
	}
	A_strncpyz( dest + l1, src, size - l1 );
}
